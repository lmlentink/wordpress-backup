jQuery(document).ready(function($) {
    ...
    if (winWidth < 1025) {
     ...
     var articleHeight = $('.single .site-main article').height();

     $('.single .post-navigation .nav-holder').stop(true, false, true).fadeOut();
     $(window).scroll(function(){
        if($(this).scrollTop() > headerHeight){
            $('.single .post-navigation .nav-holder').stop(true, false, true).fadeIn();
        }else{
            $('.single .post-navigation .nav-holder').stop(true, false, true).fadeOut();
        }

        if($(this).scrollTop() >= articleHeight){
            $('.single .post-navigation .nav-holder').stop(true, false, true).fadeOut();
        }
    });
